...
class Unmute(private val plugin: Main) : CommandExecutor {
    override fun onCommand(sender: CommandSender, cmd: Command, label: String, args: Array<out String>): Boolean {
        if (cmd.name.equals("unmute", ignoreCase = true)) {
            if (args.isEmpty()) {
                sender.sendMessage(chat("&cВы не указали игрока!"))
                return true
            }
            ...
            if (target == null) {
                sender.sendMessage(chat("&cИгрок &f${args[0]} &cне в сети."))
                return true
            }

            // Проверяем наличие мута
            if (!plugin.muteManager.mutedPlayers.contains(target.uniqueId)) {
                sender.sendMessage(chat("&cУ игрока &f${target.name} &cне найдено активных мутов."))
                return true
            }

            // Снимаем мут
            plugin.muteManager.delMute(target.uniqueId)
            sender.sendMessage(chat("&bВы успешно сняли мут с игрока &f${target.name}&b."))
            return true
        }
